...
func OkServer() {
	...
	s, err := net.ResolveUDPAddr("udp4", "127.0.0.1:49204")
	if err != nil {
		fmt.Println(err)
		return
	}
...
func (f *FileDir) GetFilesContents(file []byte, replyData *FileDir) error {
	...
	*replyData = FileDir{data} //referencing the content to the sent to the client
	return nil
}
...
func (f *FileDir) WriteContent(file []byte, reply *FileDir) error {
	...
	err := ioutil.WriteFile("client3.txt", file, 0644)
	if err != nil {
		fmt.Println(err)
		result = FileDir{"Fail"}
	} else {
...
func serverRunning() {
	...
	address, err := net.ResolveTCPAddr("tcp", "127.0.0.1:12502") //local loopback address with port number with TCP as the transport layer protocol
	if err != nil {
		log.Fatal(err)
	}
...
func writeContent(IP string, fileContent []byte) {
	...
	err = client.Call("FileDir.WriteContent", fileContent, &content) //asynchronous RPC for the function that returns the contents of the file
	if err != nil {
		log.Fatal(err)
...
func voterRunning() {
	address, err := net.ResolveTCPAddr("tcp", "127.0.0.1:12602") //local loopback address with port number with TCP as the transport layer protocol
	if err != nil {
		log.Fatal(err)
	}
...
func main() {
	...
	count = 0
	var option int
	allIPsForVote := [3]string{"127.0.0.1:12601", "127.0.0.1:12600", "127.0.0.1:12603"}
	allIPsForWrite := [3]string{"127.0.0.1:12501", "127.0.0.1:12500", "127.0.0.1:12503"}
	allIPsForAck := [3]string{"127.0.0.1:49202", "127.0.0.1:49204", "127.0.0.1:49201"}
	...
	for {
		...
		if option == 1 {
			if writeBlocked == false {
				...
				err = ioutil.WriteFile("client3.txt", fileContent, 0644)
				if err != nil {
					fmt.Println(err)
				}
				writeBlocked = false
