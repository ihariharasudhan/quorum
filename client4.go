...
func OkServer() {
	...
	s, err := net.ResolveUDPAddr("udp4", "127.0.0.1:49203")
	if err != nil {
		fmt.Println(err)
		return
	}
...
func (f *FileDir) WriteContent(file []byte, reply *FileDir) error {
	...
	err := ioutil.WriteFile("client4.txt", file, 0644)
	if err != nil {
		fmt.Println(err)
		result = FileDir{"Fail"}
	} else {
...
func serverRunning() {
	...
	address, err := net.ResolveTCPAddr("tcp", "127.0.0.1:12503") //local loopback address with port number with TCP as the transport layer protocol
	if err != nil {
		log.Fatal(err)
	}
...
func voterRunning() {
	address, err := net.ResolveTCPAddr("tcp", "127.0.0.1:12603") //local loopback address with port number with TCP as the transport layer protocol
	if err != nil {
		log.Fatal(err)
	}
	...
	forVotes := new(Vote)
	rpc.Register(forVotes) //exporting the functions of the struct FileDirs
	rpc.Accept(inbound)    //accepting connections from the clients
...
func main() {
	...
	count = 0
	var option int
	allIPsForVote := [3]string{"127.0.0.1:12601", "127.0.0.1:12602", "127.0.0.1:12600"}
	allIPsForWrite := [3]string{"127.0.0.1:12501", "127.0.0.1:12502", "127.0.0.1:12500"}
	allIPsForAck := [3]string{"127.0.0.1:49202", "127.0.0.1:49203", "127.0.0.1:49201"}
	...
	for {
		...
		if option == 1 {
			if writeBlocked == false {
				...
				err = ioutil.WriteFile("client4.txt", fileContent, 0644)
				if err != nil {
					fmt.Println(err)
				}
				writeBlocked = false
