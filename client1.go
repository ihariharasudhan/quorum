...
func OkServer() {
	...
	s, err := net.ResolveUDPAddr("udp4", "127.0.0.1:49201")
	if err != nil {
		fmt.Println(err)
		return
	}
...
func (f *FileDir) WriteContent(file []byte, reply *FileDir) error {
	...
	err := ioutil.WriteFile("client1.txt", file, 0644)
	if err != nil {
		fmt.Println(err)
		result = FileDir{"Fail"}
	} else {
...
func voterRunning() {
	address, err := net.ResolveTCPAddr("tcp", "127.0.0.1:12600") //local loopback address with port number with TCP as the transport layer protocol
	if err != nil {
		log.Fatal(err)
	}
...
func main() {
	...
	count = 0
	var option int
	allIPsForVote := [3]string{"127.0.0.1:12601", "127.0.0.1:12602", "127.0.0.1:12603"}
	allIPsForWrite := [3]string{"127.0.0.1:12501", "127.0.0.1:12502", "127.0.0.1:12503"}
	allIPsForAck := [3]string{"127.0.0.1:49202", "127.0.0.1:49203", "127.0.0.1:49204"}
	for {
		...
		if option == 1 {
			if writeBlocked == false {
				...
				if count == 3 {
					fmt.Println("Here after quorum set")
				...
				err = ioutil.WriteFile("client1.txt", fileContent, 0644)
				if err != nil {
					fmt.Println(err)
				}
				writeBlocked = false
